...
int Frontend::select_attrlist_from_join_where(
	...
	if(Algebra::join(relname_source_one, relname_source_two, ".temp", join_attr_one, join_attr_two) != SUCCESS){
		return FAILURE;
	}
	...
	int ret = Algebra::project(".temp", relname_target, attr_count, attr_list);
	if(ret != SUCCESS)	
		return FAILURE;
